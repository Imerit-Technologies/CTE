...
param AS (
    ...
    FROM (VALUES
        ROW('69653d3b2e04320329fd30cb', '697401b14a33e0719d081060', 'ango')        -- Point
        -- ROW('69653d3b2e04320329fd30cb', '6a87336c78426b7a1a987e09', 'ango'),        -- 2D Bounding Box
        -- ROW('69653d3b2e04320329fd30cb', '6979439fa2a49b048a51e016', 'ango_pct')     -- 3D Bounding Box
...
t_raw AS (
    SELECT 
        t.project_id, t.labeltaskid, t.document_id, t.page, t.object_id, 
        CASE WHEN JSON_EXTRACT_SCALAR(value, '$[0].width') IS NOT NULL THEN '2d_bounding_box' ELSE 'point' END AS coordinate_type,
        CASE 
            WHEN JSON_EXTRACT_SCALAR(value, '$[0].width') IS NOT NULL THEN
                JSON_FORMAT(CAST(MAP(
                    ARRAY['x', 'y', 'width', 'height'],
                    ARRAY[
                        CAST(JSON_EXTRACT_SCALAR(value, '$[0].x') AS DOUBLE),
                        CAST(JSON_EXTRACT_SCALAR(value, '$[0].y') AS DOUBLE),
                        CAST(JSON_EXTRACT_SCALAR(value, '$[0].width') AS DOUBLE),
                        CAST(JSON_EXTRACT_SCALAR(value, '$[0].height') AS DOUBLE)
                    ]
                ) AS JSON))
            ELSE
                JSON_FORMAT(CAST(MAP(
                    ARRAY['x', 'y'],
                    ARRAY[
                        CAST(JSON_EXTRACT_SCALAR(value, '$[0]') AS DOUBLE),
                        CAST(JSON_EXTRACT_SCALAR(value, '$[1]') AS DOUBLE)
                    ]
                ) AS JSON))
        END AS normalized_coordinates
...
gt_raw AS (
    SELECT g.project_type, g.organization_id, g.project_id, g.labeltaskid,
          g.document_id, g.stage, g.updatedby, g.updatedat_date, t.page, t.object_id, t.coordinate_type, t.normalized_coordinates
    FROM gcl_base g
...
apa_raw AS (
    SELECT 
        ...
        a.class,
        a.object_id,
        '3d_bounding_box' AS coordinate_type,
        JSON_FORMAT(CAST(
            MAP(
                ARRAY[
...
label_review_join AS (
    SELECT
        ...
        COALESCE(l.coordinate_type, r.coordinate_type) AS coordinate_type,
        l.normalized_coordinates AS normalized_coordinates_label,
        r.normalized_coordinates AS normalized_coordinates_review,
        CASE 
...
-- IOU F1 Calculation starts here --
-- IOU F1 Calculation ends here --
...
SELECT *
FROM label_review_join
-- WHERE normalized_coordinates_label <> normalized_coordinates_review AND presence_flag = 'persistent'
-- LIMIT 5
WHERE labeltaskid = '69948ac16d4c4e3b9b354785'
